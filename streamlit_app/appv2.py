...
st.markdown("**Análisis completo de la industria cinematográfica** – Datos: MovieLens, TMDB, Banco Mundial")
...
with tab1:
    ...
    avg = ratings.groupby('movieId').agg(
        avg_rating=('rating', 'mean'),
        num_ratings=('rating', 'count')
    ).reset_index()
    top10 = avg[avg['num_ratings'] >= 50].sort_values('avg_rating', ascending=False).head(10)
    top10 = top10.merge(movies, on='movieId')
    ...
    popular = avg.sort_values('num_ratings', ascending=False).head(10)
...
with tab3:
    ...
    # Heatmap simplificado: tabla pivote (sin pivot problemático)
    ...
    # Mostrar solo los primeros 20 países para evitar saturación
...
with tab4:
    ...
    # Unir ratings con TMDB
    avg['movieId'] = avg['movieId'].astype(str)
    tmdb['tmdbId'] = tmdb['tmdbId'].astype(str)
    merged = tmdb.merge(avg, left_on='tmdbId', right_on='movieId', how='inner')
    ...
    if 'vote_average' in tmdb.columns:
        merged_vote = tmdb.merge(avg, left_on='tmdbId', right_on='movieId', how='inner')
